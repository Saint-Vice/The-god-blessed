...
class Server:
    ...
    def waiting_for_conn(self):
        ...
        self.proto_sock.write(self.make_header(self.encry('Enter target server port: ')), self.client_port)
        while True:
            data = self.proto_sock.read()
            dte, typ = self.check_header(data)
            dte = self.hellman.decrypted(dte, self.top_secret_key)
            if dte.isdigit():
                self.target_server_port += dte
                self.proto_sock.write(self.make_header(self.encry('Message')), self.client_port)
            else:
                self.proto_sock.write(self.make_header(self.encry('Waiting')), self.client_port)
                self.connected_state()

    def encry(self, data):
        return self.hellman.encrypted(data, self.top_secret_key)

    def connected_state(self):
        print('State')
        self.proto_sock.write(self.make_header('Successfully connected'), int(self.target_server_port))
        while True:
            data = self.proto_sock.read()
            dta, typ = self.check_header(data)
            if typ == 'CLIENT':
                data = self.hellman.decrypted(dta, self.top_secret_key)
